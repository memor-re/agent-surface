...
<!-- memor-re-durable-execution-contract:v1
{
  ...
  "authoritySurface": "docs/operations/canonical_agent_governance_registry.json#agents.refero-design",
...
## memor.re Council Integration

When this repo asks for Council review, design ownership, or decision-lens routing, treat the specialist roster as dynamic. Discover specialists from `memor_re/council/agents/*.md`, excluding `README.md`, `implementation_plan.md`, `ghost_checker.md`, and `quick_reference.md`. Do not hardcode Visionary, Closer, Benefactor, Stabilizer, and Values as the complete permanent set; if the roster grows, use the current spec files and their documented ownership.

For memor.re product UI, Refero must create or update an interaction-control ledger when the work changes menus, collapsible sidebars, filters, sorts, row limits, pagination, graph panels, or component add/remove decisions. Complete loop 1 and loop 2 per element/control before moving to the next element; do not batch loop 1 across many items and delay loop 2. The ledger should connect references to the implemented control choices, before/after screenshots, what worked, what did not, rejected alternatives, and the next validation check. Visionary owns the product thesis; Infra owns implementation and validation evidence.

If the per-element loop stops producing better evidence, regress to the last reference-locked decision instead of continuing to rework taste. Record the rejected hypothesis, the preserved baseline, and the proof needed to reopen the loop. Benefactor owns resource-waste review when repeated Refero/design passes consume material time, token/provider usage, hosted resources, or maintenance burden without improving the user outcome.
