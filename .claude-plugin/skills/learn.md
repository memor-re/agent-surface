...
This is a memor.re canonical skill. The authored source of truth is `plugins/memor-re/skills/learn/SKILL.md`; every other location (`.agents/skills/learn/`, `.claude/skills/learn/`, the plugin bundle) is a generated byte-projection of it. It is bundled through `plugins/memor-re` and imported through the memor.re product skill pipeline when it needs to appear in cloud/user skill surfaces. Do not rely on a personal/global Codex-only copy as the source of truth.
