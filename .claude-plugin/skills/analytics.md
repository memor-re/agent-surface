...
This is a memor.re canonical skill. The authored source of truth is `plugins/memor-re/skills/analytics/SKILL.md`; every other location (`.agents/skills/analytics/`, `.claude/skills/analytics/`, the plugin bundle) is a generated byte-projection of it, so the skill doc is available as a loadable skill on every surface today. That projection availability is separate from runtime promotion: Analytics is not a Council runtime agent and must not be claimed as one until the promotion gate above is cleared.
